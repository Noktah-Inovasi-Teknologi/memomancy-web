...
<template>
  <div class="fixed top-0 left-0 right-0 z-999 mix-blend-difference">
    <div
      class="flex p-uniform-5 sm:p-uniform-4 md:p-uniform-3 font-playfair font-medium text-heading-4 sm:text-heading-3 md:text-heading-4 lg:text-heading-4 xl:text-heading-4 justify-between items-center xl:max-w-6xl xl:mx-auto"
      id="menubar"
    >
      <NuxtLink to="/" class="text-white mix-blend-difference">Memomancy</NuxtLink>

      <!-- Desktop horizontal navigation (md+) -->
      <nav class="hidden md:flex items-center gap-uniform-4 lg:gap-uniform-3 xl:gap-uniform-3">
        <NuxtLink
          v-for="route in navigationRoutes"
          :key="route.to"
          :to="route.to"
          class="text-white mix-blend-difference font-playfair text-normal-3 lg:text-normal-3 xl:text-normal-3 hover:text-gold/80 transition-colors relative nav-link"
        >
          {{ route.label }}
        </NuxtLink>
      </nav>
